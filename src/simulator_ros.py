...
class SimulatorROS:
    ...
    def send_control(self, action):
        ...
        throttle = round(float(action[0].item()), 4)
        steering = round(float(action[1].item()), 4)
        ...
        # rospy.loginfo(f"sent controls: throttle={throttle}, steering={steering}")
    ...
    def publish_path(self, U: np.array):
        ...
        # rospy.loginfo(f"[MPCC logger] states: {self.states} used for publish path")
